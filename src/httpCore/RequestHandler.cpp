...
{
    std::string target = HandlerResolver::resolvePath(req, loc);
    if (target.empty())
                                    ...
                                    const std::string& excludeFile)
{
    ...
    for (size_t i = 0; i < entries.size(); ++i)
    {
        ...
        std::string fullEntryPath = HandlerResolver::joinPath(dirPath, name);
...
static HttpResponse serveDirectory(const std::string& dirPath, const HttpRequest& req, const RuntimeLocation* loc)
{
    ...
    {
        struct stat st;
        std::string candidate = HandlerResolver::joinPath(dirPath, idx[0]);
...
HttpResponse RequestHandler::handle(const HttpRequest& req, const RuntimeLocation* loc) {
    ...
    std::string fsPath = HandlerResolver::resolvePath(req, loc);
    if (fsPath.empty())
        return ErrorHandler::build(400, loc);

    // 4- static file handling per method
    if (req.method == "GET")
